...
def readin_data(k = 52, tlag = 130, year_test_cutoff = 61):
    full_df = pd.read_parquet("../../../../output/data/basic_nn/prefit/k" + str(k) + "_tlag" + str(tlag) + ".gzip")
    train_df = full_df[full_df['time'] < year_test_cutoff]
    test_df = full_df[full_df['time'] >= year_test_cutoff]
...
def main():
    ...
    cases_train = get_data(process(train_one_city))
    cases_test = get_data(process(test_one_city))
    ...
    # save as parquet
    df.to_parquet(full_write_loc + "_fit_info.parquet")
