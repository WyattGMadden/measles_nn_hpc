...
big_city_sep_high$distance <- NA
for (i in 1:nrow(temp)) {
    temp$distance[i] <- spherical_dist(latlon$lat[latlon$city == temp$city[i]],
                                   latlon$lon[latlon$city == temp$city[i]],
                                   latlon$lat[latlon$city == temp$name[i]],
                                   latlon$lon[latlon$city == temp$name[i]])
}

write_csv(big_city_sep_high, paste0(save_dir, "k", k, "sep_high_pop_svs_rel_within_obs.csv"))
...
ggsave(paste0(save_dir, "k", k, "sep_high_pop_groups_over_pop_cut_quantile.png"), width = 9, height = 5)
