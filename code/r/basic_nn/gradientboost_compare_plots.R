...
prmse <- full_dat %>% 
    ...
    ggplot() + 
    geom_point(aes(x = gb_rmse, y = nn_rmse, colour = log(min_pop)), 
               size = 0.1) +
    geom_abline(color = "black") + 
    cividis::scale_color_cividis(direction = -1) +
    facet_wrap(~ k, ncol = 3) +
    labs(x = expression(RMSE[gbboost]),
...
rmse_by_k_over_models <- full_dat |>
    group_by(k) |>
    summarize(gb_rmse = sqrt(mean((log(gb + 1) - log(cases + 1)) ^ 2, na.rm = T)),
              nn_rmse = sqrt(mean((log(nn + 1) - log(cases + 1)) ^ 2, na.rm = T)),
              tsir_rmse = sqrt(mean((log(tsir + 1) - log(cases + 1)) ^ 2, na.rm = T)))

# save as csv
write_csv(rmse_by_k_over_models, paste0(save_dir, "gb_rmse_by_k_over_models.csv"))
...
scale_factor <- 2
ggsave(paste0(save_dir, "gb_rmse_reg_and_gain_nn_tsir_k_facet.png"),
       figrmseall, 
