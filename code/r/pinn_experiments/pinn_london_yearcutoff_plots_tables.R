...
dirs <- list.files("../../../output/models/pinn_experiments/final_london_pinn_yearcutoff", full.names = TRUE)
...
fit_info <- lapply(grep("_fit_info", dirs, value = TRUE),
                   function(x) {
                       ...
                       dat$model <- substr(x, 70, nchar(x) - 17)
                       dat$iter <- 1:nrow(dat)
                       return(dat)
                   }) |>
...
test_preds <- lapply(grep("_test_predictions", dirs, value = TRUE),
                   function(x) {
                       dat <- arrow::read_parquet(x)
                       dat$model <- substr(x, 70, nchar(x) - nchar("_test_predictions.parquet"))
                       return(dat)
                   }) |>
    (function(x) {Reduce(rbind, x)})() |>
    mutate(model = factor(model, 
...
train_preds <- lapply(grep("_train_predictions", dirs, value = TRUE),
                   function(x) {
                       dat <- arrow::read_parquet(x)
                       dat$model <- substr(x, 70, nchar(x) - nchar("_train_predictions.parquet"))
                       return(dat)
                   }) |>
    (function(x) {Reduce(rbind, x)})() |>
    mutate(model = factor(model, 
